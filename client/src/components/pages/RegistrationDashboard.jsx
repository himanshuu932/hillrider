...
const RegistrationDashboard = ({ registrationsData, onDataChange }) => {
    // --- STATE MANAGEMENT ---
    // The main data comes from props, but we keep a local copy (`registrationsState`) for UI filtering.
    ...
    // --- DATA HANDLING ---
    // This effect syncs the local state whenever the data prop from the parent component changes.
    useEffect(() => {
        setRegistrationsState(registrationsData);
    }, [registrationsData]);

    // This effect fetches school data, which is specific to this component's needs (e.g., the edit dropdown).
    useEffect(() => {
    ...
    // KPIs are calculated from the main `registrationsData` prop to always reflect the source of truth.
    const validRegistrations = useMemo(() => 
        registrationsData.filter(reg => reg.paymentStatus === 'Paid' || reg.paymentStatus === 'Offline Paid'),
    [registrationsData]);

    // Table filtering operates on the local `registrationsState`.
